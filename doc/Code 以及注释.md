...
function initialize(
    ...
    require(_startBlock <= _endBlock && _b2PerBlock > 0, "invalid parameters");  // 确保输入的区块号有效且B2代币的产出率大于0
    ...
    b2PerBlock = _b2PerBlock; // 设置每个区块产生的B2代币数量
}
```

### 2. 更新质押池的函数 `updatePool`

这个函数用于更新指定质押池的状态，包括最后奖励区块和累计每个质押代币的 B2 数量。
...
function updatePool(uint256 _pid) public checkPid(_pid) {
    ...
    uint256 b2Reward = multiplier * b2PerBlock * pool_.poolWeight / totalPoolWeight; // 根据权重计算这个池的B2奖励
    pool_.accB2PerST += b2Reward * 1e18 / stSupply; // 更新每个代币的累计B2数量
    pool_.lastRewardBlock = block.number; // 更新最后奖励区块为当前区块

    emit UpdatePool(_pid, pool_.lastRewardBlock, b2Reward); // 触发更新池的事件
}
```
